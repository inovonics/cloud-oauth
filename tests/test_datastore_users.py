...
class TestCasesDatastoreUsers(unittest.TestCase):
    ...
    def test_create_user(self, username, password, is_active, scopes):
        ...
        tmp_user_in_usernames = self.dstore.redis.sismember("oauth:users:usernames", username)
        ...
        tmp_user_in_user_ids = self.dstore.redis.sismember("oauth:users:oids", tmp_user_id)
    ...
    def test_update_user(self, username, update_field, update_value):
        ...
        if update_field == "username":
            ...
            tmp_id_check = self.dstore.redis.get("oauth:users:{}".format(update_value)).decode('utf-8')
            self.logger.debug("tmp_id_check: %s", tmp_id_check)
            self.assertEqual(tmp_id_check, tmp_user_id)
            tmp_in_set = self.dstore.redis.sismember("oauth:users:usernames", update_value)
    ...
    def _load_default_user_data(self):
        ...
        with redpipe.autoexec() as pipe:
            for user in default_user_data:
                ...
                tmp_key = "oauth:users{{{}}}".format(tmp_user_id)
                pipe.hset(tmp_key, "oid", tmp_user_id)
                ...
                pipe.set("oauth:users:{}".format(user[0]), tmp_user_id)
                # Load the username and user_id into the appropriate set
                pipe.sadd("oauth:users:usernames", user[0])
                pipe.sadd("oauth:users:oids", tmp_user_id)
